...
from loss_function.FocalCos import FocalCos
...
class ResNet1D_FocalCos(pl.LightningModule):
    """
    Modified 1D-ResNet34 with FocalCos (Learnable Uncertainty Weights) Loss
    """
    def __init__(
        self,
        in_channels: int,
        ...
        self.criterion = FocalCos(class_weights=class_weights)
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        # Log all components from loss_dict
...
if __name__ == "__main__":
    # Sanity check
    model = ResNet1D_FocalCos(in_channels=1, nb_classes=4)
    # Check if criterion has parameters
    print("Criterion parameters:", list(model.criterion.parameters()))
    
    x = torch.randn(2, 1, 300) # L=300 > tail=20
    logits = model(x)
    print("Logits shape:", logits.shape)
    
    y = torch.randint(0, 4, (2,))
    loss, loss_dict = model.criterion(logits, y)
    print("Loss:", loss)
    print("Loss dict keys:", loss_dict.keys())
    
    loss.backward()
    print("Gradient on log_vars:", model.criterion.log_vars.grad)
