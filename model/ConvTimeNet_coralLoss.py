...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None):
        ...
        out_patch = self.depatchEmbedding(x_enc)  # [bs, *, d_model]
        output = self.main_net(out_patch.permute(0, 2, 1))  # [bs, num_class]
        return output


class ConvTimeNetCoralLoss(pl.LightningModule):
    def __init__(
            self,
            # configs,
            patch_size = 16,
        ...
        # ConvTimeNet Backbone
        new_len = self.depatchEmbedding.new_len
        c_in, c_out, dropout = d_model, nb_classes-1, dropout # nb_classes-1 --> coral loss
...
# -------------------------- steps ----------------------------
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx: int = 0):
        ...
        preds = coral_decode(logits, threshold=self.threshold_coralLoss)  # (B,)
